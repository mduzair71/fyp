...
// Helper to get Backend Base URL dynamically
const getBackendUrl = () => {
...
export default function ReportPage() {
  ...
  // Dynamic Backend Live Stats State
  ...
  // Fetch Live Real Data from Database
  useEffect(() => {
    axios
      .get(`${getBackendUrl()}/issues`)
      .then((res) => {
        ...
        // Extract Unique Active Citizens
        const citizenSet = new Set();
        issues.forEach((item) => {
          if (Array.isArray(item.reported_by)) {
            item.reported_by.forEach((id) => citizenSet.add(id));
          }
          if (item.created_by) {
            citizenSet.add(item.created_by);
          }
        });
        ...
        setDbStats({
          ...
          rate: `${resolutionRate}%`,
          citizens: citizenSet.size || (total > 0 ? total : 0),
        });
      })
      .catch((err) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(`${getBackendUrl()}/issues`, form, {
        headers: { 'Content-Type': 'multipart/form-data' },
        withCredentials: true,
      });
  ...
  if (success) {
    ...
    return (
      <div className="min-h-screen w-full bg-slate-100 flex items-center justify-center px-4">
        <div className="max-w-md w-full text-center bg-white border border-slate-200 rounded-3xl shadow-xl px-8 py-12">
          <div className={`w-16 h-16 mx-auto rounded-full border-2 flex items-center justify-center mb-5 ${
            isDup ? 'bg-amber-50 border-amber-300 text-amber-600' : 'bg-emerald-50 border-emerald-300 text-emerald-600'
          }`}>
            <IconShieldCheck className="w-8 h-8" />
          </div>
          ...
          <p className="text-xs text-slate-400 mt-4 mb-6">Redirecting to issues page...</p>
          <Link
            href="/issues"
            className="inline-block px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold text-sm no-underline transition-colors shadow-sm"
          >
            View All Issues
          </Link>
        </div>
  ...
  return (
    <div className="min-h-screen w-full bg-slate-50 text-slate-900 font-['Inter',sans-serif] p-4 md:p-6 flex flex-col justify-between">
      <div className="max-w-[1440px] mx-auto w-full flex-1 flex flex-col justify-stretch">

        {/* ── 3-COLUMN LIGHT THEME GRID ── */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-stretch flex-1">

          {/* 👈 LEFT SIDEBAR */}
          <div className="lg:col-span-3 bg-white border border-slate-200 rounded-3xl p-6 shadow-sm flex flex-col justify-between overflow-hidden">
            <div>
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs font-extrabold mb-5">
                <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
                Civic Issue Reporting
              </span>
              <h1 className="text-2.5xl font-black text-slate-900 leading-tight">
                Make <span className="text-emerald-600">Your City Better</span>, One Report at a Time!
              </h1>
              ...
              {/* Highlights List */}
              <div className="flex flex-col gap-3 mt-6">
                <div className="flex items-center gap-3 p-3 rounded-2xl bg-slate-50 border border-slate-200 hover:border-emerald-300 transition-colors">
                  <div className="w-8 h-8 rounded-xl bg-emerald-100 border border-emerald-200 flex items-center justify-center text-emerald-700 text-xs font-bold shrink-0">⚡</div>
                  <div>
                    <h4 className="text-xs font-bold text-slate-900">Quick & Easy</h4>
                    <p className="text-[11px] text-slate-500">Submit issues in minutes</p>
                  </div>
                </div>

                <div className="flex items-center gap-3 p-3 rounded-2xl bg-slate-50 border border-slate-200 hover:border-emerald-300 transition-colors">
                  <div className="w-8 h-8 rounded-xl bg-blue-100 border border-blue-200 flex items-center justify-center text-blue-700 text-xs font-bold shrink-0">🎯</div>
                  <div>
                    <h4 className="text-xs font-bold text-slate-900">AI-Powered Analysis</h4>
                    <p className="text-[11px] text-slate-500">Smart categorization & priority</p>
                  </div>
                </div>

                <div className="flex items-center gap-3 p-3 rounded-2xl bg-slate-50 border border-slate-200 hover:border-emerald-300 transition-colors">
                  <div className="w-8 h-8 rounded-xl bg-teal-100 border border-teal-200 flex items-center justify-center text-teal-700 text-xs font-bold shrink-0">🕒</div>
                  <div>
                    <h4 className="text-xs font-bold text-slate-900">Real-time Tracking</h4>
                    <p className="text-[11px] text-slate-500">Track your issue progress</p>
                  </div>
                </div>

                <div className="flex items-center gap-3 p-3 rounded-2xl bg-slate-50 border border-slate-200 hover:border-emerald-300 transition-colors">
                  <div className="w-8 h-8 rounded-xl bg-amber-100 border border-amber-200 flex items-center justify-center text-amber-700 text-xs font-bold shrink-0">👥</div>
                  <div>
                    <h4 className="text-xs font-bold text-slate-900">Community Driven</h4>
                    <p className="text-[11px] text-slate-500">Support and verify together</p>
                  </div>
                </div>
              </div>

              {/* Quote */}
              <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-4 mt-5 relative">
                <span className="text-3xl font-serif text-emerald-300 absolute top-1 left-2">“</span>
                <p className="text-xs text-emerald-900 font-semibold pl-4 pt-1">
                  A better city starts with an active citizen.
                </p>
                <p className="text-[10px] text-emerald-700 font-bold text-right mt-1">— FixMyCity</p>
              </div>
            </div>

            {/* Bottom Vector Art */}
            <div className="mt-6 pt-4 border-t border-slate-100 relative">
              <svg className="w-full h-28 rounded-b-2xl opacity-75" viewBox="0 0 400 160" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M0 160H400V130C360 125 320 135 280 130C240 125 200 135 160 130C120 125 80 135 40 130L0 135V160Z" fill="#10B981" opacity="0.2" />
                <rect x="30" y="60" width="30" height="70" rx="3" fill="#10B981" opacity="0.3" />
                <rect x="70" y="40" width="45" height="90" rx="3" fill="#059669" opacity="0.3" />
                <rect x="130" y="75" width="25" height="55" rx="3" fill="#34D399" opacity="0.4" />
                <rect x="230" y="50" width="40" height="80" rx="3" fill="#10B981" opacity="0.3" />
                <rect x="280" y="30" width="50" height="100" rx="3" fill="#047857" opacity="0.3" />
                <path d="M0 145C100 135 200 155 400 140V160H0V145Z" fill="#10B981" opacity="0.4" />
                <circle cx="50" cy="120" r="14" fill="#047857" opacity="0.5" />
                <rect x="48" y="125" width="4" height="25" fill="#064E3B" opacity="0.5" />
                <circle cx="360" cy="115" r="18" fill="#065F46" opacity="0.5" />
                <rect x="358" y="122" width="4" height="28" fill="#022C22" opacity="0.5" />
              </svg>
            </div>
          </div>

          {/* 🎯 CENTER FORM CONTAINER */}
          <div className="lg:col-span-6 bg-white border border-slate-200 rounded-3xl p-6 sm:p-7 shadow-sm flex flex-col justify-between">
            
            {/* Header Stepper */}
            <div className="flex items-center justify-between mb-6 border-b border-slate-100 pb-4">
              <div className="flex items-center gap-2">
                <span className="w-7 h-7 rounded-full bg-emerald-600 text-white text-xs font-bold flex items-center justify-center shadow-sm">1</span>
                <span className="text-xs font-bold text-slate-900">Category</span>
              </div>
              <div className="h-[2px] w-8 bg-slate-200"></div>
              <div className="flex items-center gap-2">
                <span className={`w-7 h-7 rounded-full text-xs font-bold flex items-center justify-center transition-colors ${formData.title ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-500 border border-slate-200'}`}>2</span>
                <span className={`text-xs font-bold ${formData.title ? 'text-slate-900' : 'text-slate-400'}`}>Details</span>
              </div>
              <div className="h-[2px] w-8 bg-slate-200"></div>
              <div className="flex items-center gap-2">
                <span className={`w-7 h-7 rounded-full text-xs font-bold flex items-center justify-center transition-colors ${formData.location_area ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-500 border border-slate-200'}`}>3</span>
                <span className={`text-xs font-bold ${formData.location_area ? 'text-slate-900' : 'text-slate-400'}`}>Location</span>
              </div>
              <div className="h-[2px] w-8 bg-slate-200"></div>
              <div className="flex items-center gap-2">
                <span className="w-7 h-7 rounded-full bg-slate-100 border border-slate-200 text-slate-400 text-xs font-bold flex items-center justify-center">4</span>
                <span className="text-xs font-bold text-slate-400">Review</span>
              </div>
            </div>

            {error && (
              <div className="flex items-center gap-2 px-4 py-3 mb-4 rounded-xl bg-red-50 border border-red-200 text-red-700 text-xs font-medium">
                <span>⚠️</span> {error}
              </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-6">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="font-bold text-slate-900 text-sm">Issue Category</h3>
                  </div>
                  <span className="text-[10px] bg-amber-50 border border-amber-200 text-amber-700 font-bold px-2.5 py-0.5 rounded-full">🔥 Trending</span>
                </div>
                <p className="text-xs text-slate-500 mb-3">Choose the category that best matches your issue</p>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                  {Object.entries(CATEGORIES).map(([name, info]) => (
                    <button
                      ...
                      className={`relative flex flex-col items-center justify-center p-3 rounded-2xl border transition-all text-center ${
                        category === name
                          ? 'border-emerald-600 bg-emerald-50 text-emerald-900 font-bold shadow-sm scale-[1.02]'
                          : 'border-slate-200 bg-slate-50/50 hover:bg-slate-100 hover:border-slate-300 text-slate-700'
                      }`}
                    >
                      {category === name && (
                        <span className="absolute top-1.5 right-1.5 w-4 h-4 bg-emerald-600 rounded-full flex items-center justify-center text-white">
                          <IconCheck className="w-2.5 h-2.5" />
                        </span>
                      )}
                ...
                {category && problemTypes.length > 0 && (
                  <div className="mt-3.5">
                    <label className="block text-[11px] font-bold text-slate-700 mb-2">Select Problem Type</label>
                    <div className="flex flex-wrap gap-2">
                      {problemTypes.map((type) => (
                        <button
                          key={type}
              ...
              <div className="pt-4 border-t border-slate-100">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="font-bold text-slate-900 text-sm">Issue Details</h3>
                  </div>
                  <span className="text-xs text-emerald-600 font-semibold cursor-pointer hover:text-emerald-700 transition-colors">💡 Need help writing?</span>
                </div>

                <div className="flex flex-col gap-3.5 mt-2">
                  <div>
                    <div className="flex justify-between items-center mb-1">
                      <label className="text-xs font-bold text-slate-700">Title <span className="text-emerald-600">*</span></label>
                      <span className="text-[10px] text-slate-400">{formData.title.length}/100</span>
                    </div>
                    <div className="flex items-center border border-slate-200 rounded-xl bg-white focus-within:border-emerald-500 focus-within:ring-2 focus-within:ring-emerald-500/20 transition-all">
                      <IconTag className="w-4 h-4 ml-3 text-slate-400 shrink-0" />
                      <input
                        type="text"
                        name="title"
                        value={formData.title}
                        ...
                        maxLength={100}
                        placeholder="e.g., Broken street lights near Government College"
                        className="w-full bg-transparent px-3 py-2.5 text-xs outline-none text-slate-900 placeholder-slate-400"
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-1">
                      <label className="text-xs font-bold text-slate-700">Description <span className="text-emerald-600">*</span></label>
                      <span className="text-[10px] text-slate-400">{formData.description.length}/1000</span>
                    </div>
                    <div className="flex border border-slate-200 rounded-xl bg-white focus-within:border-emerald-500 focus-within:ring-2 focus-within:ring-emerald-500/20 transition-all">
                      <IconDoc className="w-4 h-4 ml-3 mt-2.5 text-slate-400 shrink-0" />
                      <textarea
                        name="description"
                        value={formData.description}
                        onChange={handleChange}
                        ...
                        rows={3}
                        placeholder="Describe the issue in detail. Mention when it started, how often it happens and its impact on the community..."
                        className="w-full bg-transparent px-3 py-2.5 text-xs outline-none text-slate-900 placeholder-slate-400 resize-none"
                      />
                    </div>
                  </div>
                </div>
              ...
              <div className="pt-4 border-t border-slate-100">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      onClick={handleUseGPS}
                      disabled={gpsLoading}
                      className="px-3 py-1.5 rounded-xl text-xs font-bold text-white bg-emerald-600 hover:bg-emerald-700 transition-colors disabled:opacity-50 flex items-center gap-1 shadow-sm"
                    >
                      📍 {gpsLoading ? 'Capturing...' : 'Use My Location'}
                    </button>
                    <button
                      type="button"
                      className="px-3 py-1.5 rounded-xl text-xs font-bold text-slate-700 border border-slate-200 bg-white hover:bg-slate-50 transition-colors flex items-center gap-1"
                    >
                      <IconMap className="w-3.5 h-3.5 text-emerald-600" /> Choose on Map
                    </button>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-start">
                  <div className="md:col-span-7 flex flex-col gap-3">
                    <select
                      ...
                      onChange={handleChange}
                      required
                      className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-xs text-slate-900 outline-none cursor-pointer focus:border-emerald-500 bg-white focus:ring-2 focus:ring-emerald-500/20 transition-all appearance-none"
                    >
                    ...
                    <select
                      ...
                      required
                      disabled={!formData.location_district}
                      className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-xs text-slate-900 outline-none cursor-pointer focus:border-emerald-500 bg-white disabled:opacity-50 disabled:cursor-not-allowed focus:ring-2 focus:ring-emerald-500/20 transition-all appearance-none"
                    >
                    ...
                    <div className="flex items-center border border-slate-200 rounded-xl bg-white focus-within:border-emerald-500 focus-within:ring-2 focus-within:ring-emerald-500/20 transition-all">
                      <IconPin className="w-4 h-4 ml-3 text-slate-400 shrink-0" />
                      <input
                        type="text"
                        name="location_landmark"
                        value={formData.location_landmark}
                  ...
                  {/* Dynamic Location Map Box */}
                  <div className="md:col-span-5 relative h-32 rounded-2xl overflow-hidden border border-slate-200 bg-slate-100 flex items-center justify-center">
                    <div className="absolute inset-0 bg-[radial-gradient(#cbd5e1_1px,transparent_1px)] [background-size:12px_12px]"></div>
                    <div className="absolute top-2 bg-white/90 backdrop-blur-md px-2.5 py-1 rounded-full text-[10px] font-bold text-slate-700 border border-slate-200 text-center shadow-sm">
                      Pin your exact location for accurate tracking
                    </div>
                    <div className="w-7 h-7 bg-emerald-600 rounded-full flex items-center justify-center text-white shadow-md z-10 animate-bounce">
                      📍
                    </div>
                  </div>
                </div>
                ...
                {formData.location_latitude && formData.location_longitude && (
                  <div className="mt-2 px-3 py-1.5 rounded-lg text-[11px] bg-emerald-50 border border-emerald-200 text-emerald-800 font-medium">
                    ✅ GPS Coordinates: {formData.location_latitude}, {formData.location_longitude}
                  </div>
                )}
              </div>
              ...
              <div className="pt-4 border-t border-slate-100 grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-2">Upload Evidence (Optional)</label>
                  <input type="file" onChange={handleFileChange} accept="image/*" className="hidden" id="file-upload" />
                  <label
                    htmlFor="file-upload"
                    className="flex flex-col items-center justify-center h-28 border-2 border-dashed border-slate-300 bg-slate-50 rounded-2xl cursor-pointer hover:border-emerald-500 hover:bg-emerald-50/50 transition-all text-center px-4"
                  >
                    {!preview ? (
                      <>
                        <IconUploadCloud className="w-5 h-5 text-slate-400 mb-1" />
                        <span className="text-[11px] font-semibold text-slate-700">Drag & drop photos or videos here or click</span>
                        <span className="text-[9px] text-slate-400 mt-0.5">JPG, PNG, GIF, WebP (Max: 5MB each)</span>
                      </>
                    ) : (
                      <div className="relative h-full w-full flex items-center justify-center p-2">
                        <img src={preview} alt="Preview" className="max-h-full rounded-lg object-contain shadow-sm" />
                        <button
                          type="button"
                          onClick={(e) => {
                            e.preventDefault();
                          ...
                          className="absolute -top-2 -right-2 w-6 h-6 rounded-full bg-red-500 border-2 border-white text-white text-[12px] flex items-center justify-center shadow hover:bg-red-600 transition-colors"
                        >
                          ✕
                        </button>
                      </div>
                ...
                <div className="flex flex-col gap-2">
                  <label className="block text-xs font-bold text-slate-700">Additional Information</label>
                  
                  <div className="flex items-center border border-slate-200 rounded-xl px-2.5 py-1.5 bg-white focus-within:border-emerald-500 transition-all">
                    <IconCalendar className="w-3.5 h-3.5 text-slate-400 mr-2 shrink-0" />
                    <input
                      type="date"
                      name="occurred_date"
                      value={formData.occurred_date}
                  ...
                  <div className="flex items-center border border-slate-200 rounded-xl px-2.5 py-1.5 bg-white focus-within:border-emerald-500 transition-all">
                    <IconClock className="w-3.5 h-3.5 text-slate-400 mr-2 shrink-0" />
                    <select
                      name="frequency"
                      value={formData.frequency}
                      onChange={handleChange}
                      className="w-full bg-transparent text-xs text-slate-900 outline-none cursor-pointer appearance-none"
                    >
                      <option value="">How often does it happen?</option>
                  ...
                  <div>
                    <span className="block text-[10px] font-bold text-slate-500 mb-1">Severity Level</span>
                    <div className="grid grid-cols-3 gap-1.5">
                      {['low', 'medium', 'high'].map((lvl) => (
                        <button
                          key={lvl}
              ...
              {/* Form Footer Action Controls */}
              <div className="flex items-center justify-between pt-2">
                <label className="flex items-center gap-2 text-xs text-slate-700 cursor-pointer group">
                  <div className="relative flex items-center justify-center w-4 h-4 rounded border border-slate-300 group-hover:border-emerald-500 transition-colors">
                    <input
                      type="checkbox"
                      name="is_anonymous"
                      checked={formData.is_anonymous}
              ...
              <div className="flex items-center gap-3 pt-3 border-t border-slate-100">
                <button
                  type="button"
                  onClick={() => router.push('/')}
                  className="px-5 py-2.5 rounded-xl font-bold text-xs text-slate-700 border border-slate-200 bg-white hover:bg-slate-50 transition-colors"
                >
                  Skip for Now
                </button>
                <button
                  type="submit"
                  disabled={!canSubmit}
                  className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl font-bold text-xs text-white bg-emerald-600 hover:bg-emerald-700 transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <IconSend className="w-4 h-4" />
                  {loading ? 'Submitting & Analyzing...' : 'Continue to Review ->'}
                </button>
          ...
          <div className="lg:col-span-3 flex flex-col justify-between gap-4">
            ...
            <div className="bg-white border border-slate-200 rounded-3xl p-5 shadow-sm">
              ...
              <ul className="flex flex-col gap-2.5 text-xs text-slate-600">
                <li className="flex items-start gap-2">
                  <span className="w-4 h-4 rounded-full bg-emerald-100 text-emerald-700 flex items-center justify-center text-[10px] font-bold shrink-0 mt-0.5">✓</span>
                  <span><strong>Be specific and clear:</strong> Provide accurate details</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="w-4 h-4 rounded-full bg-emerald-100 text-emerald-700 flex items-center justify-center text-[10px] font-bold shrink-0 mt-0.5">✓</span>
            ...
            <div className="bg-white border border-slate-200 rounded-3xl p-5 shadow-sm">
              <h3 className="text-xs font-bold text-slate-900 mb-4">What happens next?</h3>
              <div className="flex flex-col gap-3.5 relative pl-4 border-l-2 border-slate-200 ml-2">
                <div className="relative">
                  <span className="absolute -left-[21px] top-0.5 w-3 h-3 rounded-full bg-emerald-500 border border-white"></span>
                  <h4 className="text-xs font-bold text-slate-900">Issue submitted</h4>
                  <p className="text-[10px] text-slate-500">Instant confirmation</p>
                </div>
                <div className="relative">
                  <span className="absolute -left-[21px] top-0.5 w-3 h-3 rounded-full bg-emerald-600 border border-white"></span>
                  <h4 className="text-xs font-bold text-slate-900">AI analysis & categorization</h4>
                  <p className="text-[10px] text-slate-500">Smart analysis within seconds</p>
                </div>
                <div className="relative">
                  <span className="absolute -left-[21px] top-0.5 w-3 h-3 rounded-full bg-emerald-700 border border-white"></span>
                  <h4 className="text-xs font-bold text-slate-900">Sent to relevant department</h4>
                  <p className="text-[10px] text-slate-500">Based on category & location</p>
                </div>
                <div className="relative">
                  <span className="absolute -left-[21px] top-0.5 w-3 h-3 rounded-full bg-emerald-800 border border-white"></span>
                  <h4 className="text-xs font-bold text-slate-900">Authority takes action</h4>
                  <p className="text-[10px] text-slate-500">Track progress in real-time</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white border border-slate-200 rounded-3xl p-5 shadow-sm">
              <h3 className="text-[10px] font-extrabold text-emerald-700 uppercase tracking-wider mb-3">
                Together We've Reported
              </h3>
              <div className="grid grid-cols-2 gap-2.5">
                <div className="p-2.5 bg-slate-50 border border-slate-200 rounded-2xl hover:border-emerald-300 transition-colors">
                  <div className="text-base font-black text-slate-900">{dbStats.total}</div>
                  <div className="text-[10px] text-slate-500 font-semibold">Issues Reported</div>
                </div>
                <div className="p-2.5 bg-slate-50 border border-slate-200 rounded-2xl hover:border-emerald-300 transition-colors">
                  <div className="text-base font-black text-emerald-600">{dbStats.resolved}</div>
                  <div className="text-[10px] text-slate-500 font-semibold">Resolved</div>
                </div>
                <div className="p-2.5 bg-slate-50 border border-slate-200 rounded-2xl hover:border-emerald-300 transition-colors">
                  <div className="text-base font-black text-blue-600">{dbStats.rate}</div>
                  <div className="text-[10px] text-slate-500 font-semibold">Resolution Rate</div>
                </div>
                <div className="p-2.5 bg-slate-50 border border-slate-200 rounded-2xl hover:border-emerald-300 transition-colors">
                  <div className="text-base font-black text-purple-600">{dbStats.citizens}</div>
                  <div className="text-[10px] text-slate-500 font-semibold">Active Citizens</div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white border border-slate-200 rounded-3xl p-4 shadow-sm">
              ...
              <p className="text-[10px] text-slate-500 mb-2.5">Facing any issue while reporting? Our support team is here to help.</p>
              <button type="button" className="w-full py-2 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 text-xs font-bold rounded-xl border border-emerald-200 transition-colors">
                Contact Support
              </button>
            </div>
