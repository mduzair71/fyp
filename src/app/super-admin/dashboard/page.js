...
// const API_BASE = 'http://10.248.141.146:8000';
...
// // // ⚠️ Jab bhi WiFi/hotspot badle, sirf yeh ek line update karni hai
// // const API_BASE = 'http://10.248.141.146:8000';

// // // Location backend mein nested object hai: issue.location.area / issue.location.district
...
// // // photo_url / resolution_photo_url / verification_photo_url sab relative paths hain
...
// //           {/* Citizen's Original Reported Photo */}
// //           {issue.photo_url && (
// //             <div>
...
// //           {/* Admin's Resolution Evidence -- "waqai kaam hua wahan" ka proof */}
// //           <div>
...
// //           {/* Citizen's Own Verification -- Yes/No + their photo + feedback */}
// //           <div>
// //             <p className="text-[10px] font-bold uppercase text-gray-400 mb-2">Citizen Verification</p>
// //             {isResolved || isReopened ? (
// //               <div
// //                 className={`p-4 rounded-xl border ${
// //                   isResolved ? 'bg-emerald-50/60 border-emerald-100' : 'bg-orange-50/60 border-orange-100'
// //                 }`}
// //               >
// //                 <p className={`text-sm font-bold mb-2 ${isResolved ? 'text-emerald-800' : 'text-orange-800'}`}>
// //                   {isResolved
// //                     ? '✅ Citizen confirmed: Issue is actually fixed'
// //                     : '❌ Citizen reported: Issue is still NOT fixed'}
// //                 </p>
// //                 {issue.verification_feedback && (
// //                   <p className="text-sm text-slate-700 italic mb-3">💬 "{issue.verification_feedback}"</p>
// //                 )}
// //                 {issue.verification_photo_url && (
// //                   <img
// //                     src={getImageUrl(issue.verification_photo_url)}
// //                     alt="Citizen verification evidence"
...
// //                 {isAwaitingVerification
// //                   ? 'Waiting for citizen to verify the resolution.'
...
// //   // Robust Case-Insensitive Filtering (location ab nested object se aata hai)
// //   const filteredIssues = issues.filter((i) => {
// //     const matchesSearch =
// //       i.title?.toLowerCase().includes(search.toLowerCase()) ||
// //       i.description?.toLowerCase().includes(search.toLowerCase()) ||
...
// //   // Har department ka alag-alag performance -- poore system ka data (filters se independent)
// //   const getDepartmentBreakdown = () => {
// //     return departmentList
// //       .map((dept) => {
// //         const deptIssues = issues.filter(
// //           (i) => (i.category || '').trim().toLowerCase() === dept.trim().toLowerCase()
// //         );
...
// //         const ratio = total > 0 ? Math.round((resolved / total) * 100) : 0;
// //         return { dept, total, pending, inProgress, awaitingVerification, resolved, reopened, ratio };
// //       })
...
// //             {/* Performance Panel (respects filters above) */}
// //             <div className="bg-white border border-emerald-100 rounded-3xl p-6 mb-8 shadow-sm">
// //               <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-100 pb-5 mb-6">
// //                 <div>
// //                   <div className="flex items-center gap-2">
// //                     <span className="text-xl">📊</span>
// //                     <h2 className="text-xl font-extrabold text-slate-900">Filtered Performance & Resolution Ratio</h2>
// //                   </div>
// //                   <p className="text-slate-500 text-xs mt-1">
// //                     Scope: <strong className="text-emerald-700">{districtFilter === 'ALL' ? 'All Districts' : districtFilter} {areaFilter !== 'ALL' && `(${areaFilter})`}</strong> | Dept: <strong className="text-emerald-700">{deptFilter === 'ALL' ? 'All Departments' : deptFilter}</strong>
// //                   </p>
...
// //             {/* Per-Department Performance Table -- ALL departments side by side, system-wide */}
// //             <div className="bg-white border border-emerald-100 rounded-2xl p-6 mb-8 shadow-sm">
// //               <div className="mb-4">
// //                 <h2 className="text-lg font-bold text-slate-900 flex items-center gap-2">🏢 All Departments Performance</h2>
// //                 <p className="text-slate-500 text-xs mt-0.5">
// //                   Resolution ratio & status breakdown for every department (system-wide, ignores filters above).
// //                 </p>
// //               </div>
...
// //                         <td className="py-3 pr-3 font-bold text-slate-900">{row.dept}</td>
// //                         <td className="py-3 px-3 text-center text-slate-700">{row.total}</td>
