...
//     { href: '/issues', label: 'Explore Compliants', icon: '🧩' },
//     { href: '/map', label: 'Map', icon: '🗺️' },
//     { href: '/issues/my-issues', label: 'Track Your Compliant', icon: '📋' },
...
export default function Navbar() {
  ...
  // Dynamic pending verifications track karne ke liye state
  ...
  const handleNotificationClick = () => {
    if (pendingVerifications.length > 0) {
      // Direct notification se sab se pehle pending issue ID ke verify page par route karega
      const issueToVerify = pendingVerifications[0];
      const targetId = issueToVerify._id || issueToVerify.id;
      router.push(`/issues/${targetId}/verify`);
    } else {
      // Clear flow fallback jab verify karne ke liye koi issue na ho
      ...
      <nav className="sticky top-0 z-50 w-full">
        <div className="flex items-center justify-between bg-white/95 backdrop-blur-md border-b border-slate-200/80 shadow-sm px-6 sm:px-10 h-16 w-full">
          ...
          <div className="flex items-center gap-2.5 shrink-0">
            ...
            {/* Notification Bell Connected to Verification Flow */}
            {mounted && userName && (
              <button
                onClick={handleNotificationClick}
                className="relative p-2 rounded-full hover:bg-emerald-50 text-slate-600 hover:text-emerald-700 transition-colors cursor-pointer border-none bg-transparent active:scale-95"
                title={
                  pendingVerifications.length > 0 
                    ? `${pendingVerifications.length} pending issue verification required` 
